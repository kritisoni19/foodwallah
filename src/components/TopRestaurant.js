...
function TopRestaurant() {
  const [showCarousalImage] = useState(swiggyapidata.restaurants);
          ...
          <div  className="grid_property mb-3">
            {
              ...
              return (
                <div key={index}>
                  ...
                  />
                  <div className="mt-3">
                    <h4 className="fs-5">{e?.info?.name}</h4>
                    <div className="d-flex mb-2">
                        ...
                        <span>&nbsp; {e?.info?.avgRating} . {new Date(e?.info?.availability?.nextCloseTime).getMinutes()} min </span>
                    </div>
                    <p className="p-mt">{e?.info?.cuisines.join(', ')}</p>
                    <p className="p-mt">{e?.info?.areaName}</p>
