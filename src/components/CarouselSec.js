...
function CarouselSec() {
  const [showCarSecImg, setshowCarSecImg] = useState([]);

  useEffect(() => {
    getSecCarousalData();
  }, []);

  const getSecCarousalData = async () => {
    const data = await fetch(SWIGGY_API);
    const json = await data.json(data);
    // console.log(json?.data);

    setshowCarSecImg(json?.data?.cards[1]?.card?.card);
  };
        ...
        <div className="row ">
          <div className="col-md-12 col-sm-12">
            <div className="d-flex align-items-center justify-content-between">
              <h2 className="page_heading">{showCarSecImg?.header?.title}</h2>
              <div className="me-5">
                <span>
                  <svg
                    width="30"
            ...
            <div className="whats_scroll">
              {showCarSecImg?.imageGridCards?.info.map((e, id) => {
                return (
                  <img
                    key={id}
                    src={SWIGGY_CAROUSEL_IMG + e.imageId}
                    style={{
                      height: "180px",
                      width: "144px",
                      paddingRight: "16px",
